    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.3fr_.7fr]">
      <div className="space-y-5">
        ...
        {(goals||[]).map(goal => {
          ...
          return <article key={goal.id} className="card border-blue-100 p-5 sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="text-xl font-semibold">{goal.title}</p><Badge>{goalTypeLabel((goal.goal_type||goal.category||"personal") as GoalKind)}</Badge><Badge neutral>{mode==="frequency"?"Routine":mode==="number"?"Measured target":"Milestone plan"}</Badge></div><p className="mt-3 max-w-2xl text-sm leading-6 text-black/55">{goal.summary}</p><p className="mt-3 text-sm font-medium text-blue-800">Success looks like: {goal.success_definition||"Meaningful progress you can review each week."}</p></div><span className="text-2xl font-semibold text-blue-700">{goal.progress||0}%</span></div>
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-blue-50"><div className="h-full rounded-full bg-blue-600" style={{width:`${goal.progress||0}%`}}/></div>
            {mode==="frequency" ? <div className="mt-5 rounded-2xl bg-blue-50/60 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">This week: {weeklyCount} of {goal.weekly_frequency||3} check-ins</p><p className="mt-1 text-sm text-black/50">Use a check-in whenever you complete the routine. No artificial finish date required.</p></div><form action={checkInGoal}><input type="hidden" name="goal_id" value={goal.id}/><SubmitButton className="button-primary" pendingText="Recording...">I did this today</SubmitButton></form></div></div> : null}
            {mode==="number" ? <form action={updateGoal} className="mt-5 grid gap-3 rounded-2xl bg-blue-50/50 p-4 sm:grid-cols-3"><input type="hidden" name="id" value={goal.id}/><input type="hidden" name="tracking_mode" value={mode}/><Field label={`Current ${goal.unit||"amount"}`}><input className="input" type="number" step="0.01" name="current_value" defaultValue={goal.current_value||0}/></Field><Field label={`Target ${goal.unit||"amount"}`}><input className="input" type="number" step="0.01" name="target_value" defaultValue={goal.target_value||0}/></Field><Field label="Status"><select className="input" name="status" defaultValue={goal.status}><option value="active">Active</option><option value="completed">Completed</option><option value="paused">Paused</option></select></Field><SubmitButton className="button-primary sm:col-span-3 sm:w-fit" pendingText="Updating...">Update goal</SubmitButton></form> : <form action={updateGoal} className="mt-5 flex flex-col gap-3 rounded-2xl bg-blue-50/50 p-4 sm:flex-row sm:items-end"><input type="hidden" name="id" value={goal.id}/><input type="hidden" name="tracking_mode" value={mode}/><input type="hidden" name="current_value" value="0"/><input type="hidden" name="target_value" value="0"/><Field label="Overall progress"><input className="input" type="number" min="0" max="100" name="progress" defaultValue={goal.progress||0}/></Field><Field label="Status"><select className="input" name="status" defaultValue={goal.status}><option value="active">Active</option><option value="completed">Completed</option><option value="paused">Paused</option></select></Field><SubmitButton className="button-primary" pendingText="Updating...">Update goal</SubmitButton></form>}
            <div className="mt-5 space-y-2">{goalSteps.map(step=><div key={step.id} className="flex items-center justify-between gap-3 rounded-xl border border-black/8 p-3"><div><p className={`text-sm font-medium ${step.completed_at?"line-through text-black/35":""}`}>{step.title}</p>{step.due_date?<p className="text-xs text-black/40">Due {formatDate(step.due_date)}</p>:null}</div><form action={toggleStep}><input type="hidden" name="id" value={step.id}/><input type="hidden" name="complete" value={step.completed_at?"false":"true"}/><button className="text-xs font-semibold text-blue-700">{step.completed_at?"Reopen":"Complete"}</button></form></div>)}</div>
...
function Metric({label,value,detail}:{label:string;value:string;detail:string}){return <div className="card border-blue-100 p-5"><p className="text-sm text-black/45">{label}</p><p className="mt-2 truncate text-2xl font-semibold">{value}</p><p className="mt-2 text-xs text-black/45">{detail}</p></div>}
function Badge({children,neutral=false}:{children:React.ReactNode;neutral?:boolean}){return <span className={`rounded-full px-3 py-1 text-xs font-semibold ${neutral?"bg-black/5 text-black/55":"bg-blue-50 text-blue-700"}`}>{children}</span>}
