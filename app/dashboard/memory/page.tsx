  ...
  return <section className="mx-auto max-w-6xl pb-16">
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[.85fr_1.15fr]">
      <div className="space-y-6">
        <form action={addMemory} className="card border-blue-100 p-6"><p className="eyebrow">Tell Joye directly</p><h2 className="mt-2 text-2xl font-semibold">What should Joye remember?</h2><textarea className="input mt-5 min-h-28" name="text" placeholder="Example: I prefer planning difficult tasks in the morning." maxLength={800}/><SubmitButton className="button-primary mt-4" pendingText="Saving memory...">Remember this</SubmitButton></form>
        <div className="card border-blue-100 p-6"><div className="flex items-center gap-3"><BookOpenText className="text-blue-600"/><div><p className="eyebrow">Saved memories</p><h2 className="mt-1 text-2xl font-semibold">Your editable memory</h2></div></div><div className="mt-5 space-y-3">{context.memories.length ? context.memories.map((memory) => <article key={memory.id} className="rounded-2xl border border-black/8 bg-white p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-blue-600">{memory.label}</p><p className="mt-2 break-anywhere text-sm leading-6 text-black/65">{memory.text}</p><p className="mt-2 text-xs text-black/35">Saved {new Date(memory.createdAt).toLocaleDateString()}</p></div><form action={deleteMemory}><input type="hidden" name="id" value={memory.id}/><button aria-label="Delete memory" className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-red-50 text-red-600"><Trash2 size={16}/></button></form></div></article>) : <p className="rounded-2xl bg-blue-50 p-4 text-sm text-blue-900/65">No personal notes yet. Your goals, money, career, and weekly data are still read directly whenever Joye builds guidance.</p>}</div></div>
      </div>
      <div className="card border-blue-100 p-6 sm:p-8"><p className="eyebrow">The Journey</p><h2 className="mt-2 text-3xl font-semibold">Recent progress across your life</h2><div className="mt-7 space-y-0">{timeline.length ? timeline.map((item, index) => { const Icon = item.icon; return <div key={item.id} className="relative flex gap-4 pb-7"><div className="relative z-10 grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-blue-50 text-blue-700"><Icon size={18}/></div>{index < timeline.length - 1 ? <div className="absolute left-[21px] top-11 h-full w-px bg-blue-100"/> : null}<div className="pt-1"><p className="text-xs font-semibold uppercase tracking-wide text-blue-600">{item.kind}</p><p className="mt-1 font-semibold">{item.title}</p><p className="mt-1 text-xs text-black/40">{new Date(item.date!).toLocaleDateString()}</p></div></div>}) : <div className="rounded-2xl bg-blue-50 p-5 text-sm text-blue-900/65">Complete goals, career evidence, bills, or weekly actions to begin your Journey timeline.</div>}</div></div>
    </div>
  </section>;
}

function Snapshot({ icon: Icon, label, value }: { icon: typeof Target; label: string; value: string }) { return <div className="card min-w-0 border-blue-100 p-5"><Icon className="text-blue-600" size={19}/><p className="mt-4 text-sm text-black/45">{label}</p><p className="mt-2 break-anywhere font-semibold leading-6">{value}</p></div>; }
