  ...
  return <section className="mx-auto max-w-7xl pb-16">
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
      <div className="space-y-6">
        <div className="card border-blue-100 p-6 sm:p-8">
          ...
          {next ? <form action={addToWeek} className="mt-5 flex flex-col gap-3 rounded-2xl bg-blue-50/70 p-4 sm:flex-row sm:items-end">
        ...
        <div className="card border-blue-100 p-6 sm:p-8">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">{gaps.map((gap: string, index: number) => <div key={gap} className="rounded-2xl border border-blue-100 bg-blue-50/45 p-4"><span className="text-xs font-bold text-blue-600">GAP {index + 1}</span><p className="mt-2 font-semibold leading-6">{gap}</p></div>)}</div>
        </div>

        <div className="card border-blue-100 p-6 sm:p-8">
          <p className="eyebrow">Your career path</p><h2 className="mt-2 text-2xl font-semibold">Three phases, in the right order</h2>
          <div className="mt-6 space-y-4">{phases.map((phase, index) => <article key={phase.name} className="rounded-2xl border border-black/8 p-5"><div className="flex items-start gap-4"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-blue-600 font-bold text-white">{index + 1}</div><div><h3 className="text-lg font-semibold">{phase.name.replace(/^\d\. /, "")}</h3><p className="mt-1 text-sm text-black/45">{phase.detail}</p><ul className="mt-4 space-y-2">{phase.items.map((item: string) => <li key={item} className="flex gap-2 text-sm leading-6 text-black/65"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-blue-500" />{item}</li>)}</ul></div></div></article>)}</div>
        </div>
...
function GuidedSetup({ plan, params }: { plan: any; params: { saved?: string; error?: string } }) {
  return <section className="mx-auto max-w-4xl pb-16">
    ...
    <FlashMessage saved={params.saved} error={params.error} />
    <form action={completeCareerSetup} className="card mt-8 border-blue-100 p-6 sm:p-9">
      ...
      <div className="mt-8 rounded-2xl bg-blue-50/70 p-5"><p className="font-semibold text-blue-950">What Joye will build</p><p className="mt-2 text-sm leading-6 text-blue-900/65">A clear direction, personalized gaps, a three-phase path, the first actions to take, and a place to save career wins and proof.</p></div>
...
function Metric({ label, value, detail }: { label: string; value: string; detail: string }) { return <div className="card border-blue-100 p-5"><p className="text-sm text-black/45">{label}</p><p className="mt-2 text-xl font-semibold leading-7">{value}</p><p className="mt-2 text-xs leading-5 text-black/45">{detail}</p></div>; }
