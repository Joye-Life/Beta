...
export function CheckInReminder({ intervalHours, lastCheckInAt, primaryFocus }: { intervalHours: number; lastCheckInAt: string | null; primaryFocus: string }) {
  ...
  if (!show) return null;
  return <div className="mb-6 flex flex-col gap-4 rounded-3xl border border-joye-200 bg-joye-50 p-5 sm:flex-row sm:items-center sm:justify-between">
    <div className="flex gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl brand-gradient text-white"><Bell size={18}/></span><div><p className="font-semibold">Time for a quick check-in</p><p className="mt-1 text-sm text-black/55">Review what changed and make sure today’s next move still fits.</p></div></div>
