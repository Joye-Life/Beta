...
export async function TodayExperience() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    <section className="card overflow-hidden border-blue-100">
      <div className="bg-gradient-to-br from-blue-700 to-blue-500 p-6 text-white sm:p-9">
    ...
    <section className="card flex flex-col gap-5 border-blue-100 bg-gradient-to-br from-blue-50 to-white p-6 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-blue-600 text-white"><Brain size={20}/></span><div><p className="eyebrow">Ask Joye</p><h2 className="mt-1 text-2xl font-semibold">Talk through your next decision.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-black/55">Joye already has your saved goals, money, career, weekly plan, and memory. You do not need to explain everything again.</p></div></div><Link href="/dashboard/coach?section=today" className="button-primary shrink-0 gap-2">Ask about today <ArrowRight size={16}/></Link></section>

    <section><div className="mb-4"><p className="eyebrow">Joye signals</p><h2 className="mt-2 text-2xl font-semibold">What matters right now</h2></div><div className="grid gap-4 md:grid-cols-2">{brief.signals.map(signal=>{const Icon=icons[signal.kind];return <article key={signal.id} className="card border-blue-100 p-5"><div className="flex gap-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-blue-50 text-blue-700"><Icon size={18}/></span><div><p className="font-semibold">{signal.title}</p><p className="mt-2 text-sm leading-6 text-black/55">{signal.detail}</p></div></div></article>})}<article className="card border-blue-100 p-5"><div className="flex gap-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-green-50 text-green-700"><CheckCircle2 size={18}/></span><div><p className="font-semibold">Built from your account</p><p className="mt-2 text-sm leading-6 text-black/55">Joye uses your saved profile, tasks, goals, payments, money, and career information.</p></div></div></article></div></section>
  </div>;
}
